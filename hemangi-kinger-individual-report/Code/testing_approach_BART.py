...
from transformers import pipeline, GPT2Model, GPT2Tokenizer, GPT2Config, GPT2LMHeadModel
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df, tokenizer, max_len, max_tgt_len):
...
train_dataset = CustomDataset(train_df, tokenizer, MAX_LEN, MAX_TARGET_LEN)
valid_dataset = CustomDataset(val_df, tokenizer, MAX_LEN,MAX_TARGET_LEN)
test_dataset = CustomDataset(test_df, tokenizer, MAX_LEN, MAX_TARGET_LEN)
...
def train_and_val(train_dataloader, val_dataloader):
    ...
    # plt.plot(df_stats['Training Loss'], marker='o', color='blue', label="Training")
    # plt.plot(df_stats['Valid. Loss'], marker='o', color='red', label="Validation")
    plt.plot(df_stats['ROUGE1'], marker='*' , label="ROUGE1")
    plt.plot(df_stats['ROUGE2'], marker='*', label="ROUGE2")
    # plt.plot(df_stats['ROUGE3'], marker='*', label="ROUGE3")
    plt.plot(df_stats['ROUGEL'], marker='*',  label="ROUGEL")
    plt.plot(df_stats['ROUGELSUM'], marker='*', label="ROUGELSUM")
...
def shap_explaination(train_df, trained_model, tokenizer):
...
shap_explaination (train_df, trained_model, tokenizer)